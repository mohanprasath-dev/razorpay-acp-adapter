...
> **Spec-compliant Agentic Commerce Protocol (ACP) checkout adapter backed by Razorpay.**  
...
**The Solution**: The **Razorpay ACP Adapter** bridges the open **Agentic Commerce Protocol (ACP `v2026-04-17`)** to Razorpay's battle-tested payment rail. It enforces:
- **Server-Authoritative Pricing**: Catalog lookups ignore client-supplied unit prices.
- **Deterministic Guardrails**: Hard limits on maximum discounts (50%), order values (₹50,000), and quantities (10 units).
...
- **Live Next.js 14 Dashboard**: Real-time monitoring and inspection for merchant operators.
...
Run the complete test suite (41 unit and integration tests):
```bash
pytest
```
...
| **Max Order Value** | Capped at ₹50,000 INR | `HTTP 400` + Session status `rejected` + Logged Reason |
